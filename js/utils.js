...
// 全角カタカナ→ひらがな変換。Unicodeのカタカナ(U+30A1-U+30F6)は同じ並び順で
// ひらがな(U+3041-U+3096)より0x60大きいコードポイントに配置されているため、
// 単純に0x60引くだけで変換できる(長音記号「ー」など対応範囲外の文字はそのまま)。
function katakanaToHiragana(str) {
  return (str || '').replace(/[\u30A1-\u30F6]/g, ch => String.fromCharCode(ch.charCodeAt(0) - 0x60));
}

// 曲名検索(オートコンプリート)用の正規化。normalizeString の処理(全角英数字→半角・
// 小文字化・空白等除去)に加えて、カタカナ→ひらがな変換を行う。これにより、
// 曲名(漢字/カタカナ/英数字混在)と読み方(ひらがな)の双方を同じ土俵で比較でき、
// ひらがな入力・カタカナ入力のどちらで検索しても一致するようになる。
function normalizeKanaSearch(str) {
  if (!str) return "";
  return katakanaToHiragana(normalizeString(str));
}
