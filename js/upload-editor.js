...
async function handleFiles(files) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const item = {
      ...
      data: {
        title: '', level: '', diff: 'MS',
        perfect: 0, great: 0, good: 0, bad: 0, missDetail: 0, totalMiss: 0, totalNotes: 0,
  ...
  for (const rec of targets) {
    ...
    editorQueue.push({
      ...
      data: {
        ...
        totalNotes: rec.totalNotes || (rec.perfect + rec.great + rec.good + rec.bad + rec.miss),
        combo: rec.combo,
        musicId: rec.musicId,
        deviceProfileId: profile ? profile.id : null,
      },
...
function formatRegionInfo(region, imageSize) {
  if (!region || !imageSize) return '';
  const xPx = Math.round((region.x || 0) * (imageSize.width || 0));
  const yPx = Math.round((region.y || 0) * (imageSize.height || 0));
  const wPx = Math.round((region.w || 0) * (imageSize.width || 0));
  const hPx = Math.round((region.h || 0) * (imageSize.height || 0));
  const x = Number(region.x || 0).toFixed(3);
  const y = Number(region.y || 0).toFixed(3);
  const w = Number(region.w || 0).toFixed(3);
  const h = Number(region.h || 0).toFixed(3);
  return `${xPx}px, ${yPx}px, ${wPx}px, ${hPx}px / ${x}, ${y}, ${w}, ${h}`;
}

function renderVariantListHtml(variants, selectedText) {
  if (!variants || variants.length === 0) return '<div class="ocr-log-empty">ログなし</div>';
  return variants.map(v => {
    const score = Number.isFinite(v.score) ? v.score.toFixed(1) : '0.0';
    const conf = Number.isFinite(v.confidence) ? v.confidence.toFixed(1) : '0.0';
    const active = v.text === selectedText ? ' active' : '';
    const extraParts = [];
    if (v.extra && v.extra.diffCode) extraParts.push(`判定:${escapeHtml(v.extra.diffCode)}`);
    if (v.extra && v.extra.parsedCombo) extraParts.push(`抽出:${v.extra.parsedCombo}`);
    if (v.extra && v.extra.titleMatch && v.extra.titleMatch.title) extraParts.push(`候補:${escapeHtml(v.extra.titleMatch.title)}`);
    return `
      <div class="ocr-log-variant${active}">
        <div class="ocr-log-variant-head">
          <span class="ocr-log-pill">${escapeHtml(v.variant)}</span>
          <span>score ${score} / conf ${conf}</span>
        </div>
        <div class="ocr-log-variant-text">${escapeHtml(v.text || '（空）')}</div>
        ${extraParts.length ? `<div class="ocr-log-variant-meta">${extraParts.join(' / ')}</div>` : ''}
        ${v.previewDataUrl ? `<img class="ocr-log-preview" src="${v.previewDataUrl}" alt="OCR preview">` : ''}
      </div>
    `;
  }).join('');
}
...
function renderOcrDebugPanel(item) {
  ...
  if (!item || !item.analysisLog) {
    content.innerHTML = '<div class="ocr-log-empty">画像を選択すると、座標・二値化画像・OCR結果を表示します。</div>';
    panel.style.display = 'block';
    return;
  }

  const log = item.analysisLog;
  const imageSize = log.imageSize || { width: 0, height: 0 };
  ...
  content.innerHTML = `
    <div class="ocr-log-summary">
      <div><strong>画像サイズ</strong>: ${imageSize.width} × ${imageSize.height}</div>
      <div><strong>難易度</strong>: ${escapeHtml(summary.diffCode || '')} / <strong>レベル</strong>: ${escapeHtml(summary.level || '')} / <strong>総ノーツ数</strong>: ${escapeHtml(summary.totalNotes !== undefined ? String(summary.totalNotes) : '')}</div>
      <div><strong>タイトル補完</strong>: ${escapeHtml(summary.titleMode || '')} / score ${Number.isFinite(summary.titleScore) ? summary.titleScore.toFixed(2) : '-'}</div>
    </div>
    ${sections.map(([label, entry]) => {
      if (!entry) return '';
      const imageSizeText = formatRegionInfo(entry.region, imageSize);
      const best = entry.best || {};
      return `
        <details class="ocr-log-section" open>
          <summary>${label}</summary>
          <div class="ocr-log-region">座標: ${escapeHtml(imageSizeText)}</div>
          <div class="ocr-log-region">最終: ${escapeHtml(best.text || JSON.stringify(entry.finalValue || ''))}</div>
          <div class="ocr-log-region">前処理: ${escapeHtml(best.variant || '')} / conf ${Number.isFinite(best.confidence) ? best.confidence.toFixed(1) : '0.0'}</div>
          ${best.previewDataUrl ? `<img class="ocr-log-preview-large" src="${best.previewDataUrl}" alt="${escapeHtml(label)} preview">` : ''}
          <div class="ocr-log-variants">${renderVariantListHtml(entry.variants, best.text)}</div>
        </details>
      `;
    }).join('')}
  `;
  ...
  for (const item of itemsToAnalyze) {
    ...
    try {
      ...
      if (res) {
        ...
        item.analysisLog = res.debug || null;
        item.status = 'done';
      } else {
        item.status = 'error';
      }
    ...
    if (item.status === 'done') {
      ...
      if (activeItemId === item.id) {
        ...
        const activeItem = editorQueue.find(q => q.id === activeItemId);
        renderOcrDebugPanel(activeItem || null);
      }
    } else {
