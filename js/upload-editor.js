  ...
  for (const rec of targets) {
    ...
    editorQueue.push({
      ...
      data: {
        ...
        totalNotes: rec.perfect + rec.great + rec.good + rec.bad + rec.miss,
        combo: rec.combo,
        musicId: rec.musicId,
        deviceProfileId: profile ? profile.id : null,
      },
...
function clearDebugUrls(item) {
  if (!item || !item.debug || !item.debug.fields) return;
  Object.values(item.debug.fields).forEach(field => {
    if (field && field.blobUrl) {
      try { URL.revokeObjectURL(field.blobUrl); } catch (e) {}
    }
  });
}
...
function renderDebugPanel(item) {
  ...
  if (!item) {
    meta.innerText = '解析ログはありません';
    body.innerHTML = '<div class="ocr-debug-empty">画像を解析すると、座標・二値化画像・OCR結果が表示されます。</div>';
    return;
  }
  ...
  const totalNotes = item.data.totalNotes || 0;
  meta.innerText = `総ノーツ数: ${totalNotes} / 難易度: ${item.data.diff || '-'} / レベル: ${item.data.level || '-'} / 曲名候補: ${(summary.titleCandidates && summary.titleCandidates[0]) ? summary.titleCandidates[0].title : 'なし'}`;

  const order = ['difficulty', 'title', 'level', 'breakdown', 'combo'];
  body.innerHTML = order.map(key => {
    const field = fields[key];
    if (!field) {
      return `<div class="ocr-debug-card"><div class="ocr-debug-card-title">${key}</div><div class="ocr-debug-empty-small">ログなし</div></div>`;
    }
    const region = field.region || {};
    const parsed = typeof field.parsed === 'object' ? `<pre>${escapeHtml(JSON.stringify(field.parsed, null, 2))}</pre>` : escapeHtml(field.parsed ?? '');
    const conf = field.confidence !== null && field.confidence !== undefined ? `${Math.round(field.confidence)}%` : '-';
    return `
      <div class="ocr-debug-card">
        <div class="ocr-debug-card-title">${REGION_DEFS.find(d => d.key === key)?.label || key}</div>
        <div class="ocr-debug-card-grid">
          <div class="ocr-debug-thumb-wrap">
            ${field.blobUrl ? `<img src="${field.blobUrl}" class="ocr-debug-thumb" alt="${key}">` : '<div class="ocr-debug-empty-small">画像なし</div>'}
          </div>
          <div class="ocr-debug-text">
            <div><strong>座標</strong> x:${(region.x ?? 0).toFixed(3)} y:${(region.y ?? 0).toFixed(3)} w:${(region.w ?? 0).toFixed(3)} h:${(region.h ?? 0).toFixed(3)}${item.imageWidth && item.imageHeight ? ` (${Math.round((region.x || 0) * item.imageWidth)}px, ${Math.round((region.y || 0) * item.imageHeight)}px, ${Math.round((region.w || 0) * item.imageWidth)}px, ${Math.round((region.h || 0) * item.imageHeight)}px)` : ''}</div>
            <div><strong>OCR</strong> ${escapeHtml(field.rawText || '') || '（空）'}</div>
            <div><strong>信頼度</strong> ${conf}</div>
            <div><strong>抽出</strong> ${parsed || '（なし）'}</div>
            ${field.threshold !== undefined ? `<div><strong>閾値</strong> ${field.threshold}</div>` : ''}
            ${field.cropSize ? `<div><strong>二値化画像</strong> ${field.cropSize.width}×${field.cropSize.height}px</div>` : ''}
            ${field.correctedBy ? `<div><strong>補正</strong> ${escapeHtml(field.correctedBy)}</div>` : ''}
          </div>
        </div>
      </div>`;
  }).join('');

  const jsonEl = document.getElementById('ocr-debug-json');
  if (jsonEl) jsonEl.textContent = JSON.stringify({
    title: item.data.title,
    level: item.data.level,
    diff: item.data.diff,
    totalNotes: item.data.totalNotes,
