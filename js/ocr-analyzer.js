...
//
// onFieldDone を渡すと、5項目(難易度/レベル/曲名/判定内訳/コンボ数)を読み取るたびに
// 呼び出される。呼び出し側(upload-editor.js)はこれを使って解析の進捗バーを更新する。
async function analyzeLoadedImage(imgElement, worker, regions, onFieldDone) {
  ...
  function logField(key, prep, ocrResult, extra) {
    ...
    if (onFieldDone) onFieldDone(key);
  }
