...
function buildDriveProperties(data) {
  const perfect = toInt(data.perfect, 0);
  const great = toInt(data.great, 0);
  const good = toInt(data.good, 0);
  const bad = toInt(data.bad, 0);
  const miss = toInt(data.miss !== undefined ? data.miss : data.missDetail, 0);
  const missCount = good + bad + miss;
  const totalNotes = toInt(
    data.totalNotes,
...
function parseDriveProperties(props) {
  props = props || {};
  const perfect = toInt(props.pf, 0);
  const great = toInt(props.gr, 0);
  const good = toInt(props.gd, 0);
  const bad = toInt(props.bd, 0);
  const miss = toInt(props.ms, 0);
  const mcRaw = props.mc;
  const missCount = (mcRaw !== undefined && mcRaw !== '') ? toInt(mcRaw, good + bad + miss) : (good + bad + miss);
  const totalNotes = toInt(props.tn, perfect + great + good + bad + miss);
  return {
...
function buildResultFileName(data, ext) {
  const miss = data.miss !== undefined ? data.miss : data.missDetail;
  const missCount = toInt(data.totalMiss, toInt(data.good, 0) + toInt(data.bad, 0) + toInt(miss, 0));
