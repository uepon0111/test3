 ...
 *
 * 注意: これより前のバージョンにあった「ルート > FC > "{レベル}{難易度} 曲名"
 * フォルダ > FCファイル」という旧命名法の読み込み・書き込みサポートは廃止しました。
 * Drive のアクセス範囲を drive.file スコープ(アプリが作成したファイルのみ)に
 * 縮小したことに伴い、以前の広い検索(ファイル名に "FC" を含むものを全文検索する等)を
 * 維持する意味が薄れたためです。旧方式のデータはこのバージョンの一覧には表示されなく
 * なります(画像自体はGoogle Drive上にそのまま残るため、必要であればDriveアプリから
 * 直接確認できます)。
 * -----------------------------------------------------------------------
 */
...
async function fetchAllRecords() {
  ...
  if (!root) return [];
  return await fetchNewSchemeRecordsForRoot(root.id);
}
...
// onProgress(0〜1) を渡すとアップロードの送信進捗を通知する(uploadFileToResults 参照)。
...
// 既存レコードの更新(メタデータ更新のみ。画像本体は変わらないため再アップロードしない)。
