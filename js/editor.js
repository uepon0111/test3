...
const EditorScreen = (() => {
  ...
  function _chip(label, type, onRemove, color) {
  ...
  function _bindTagInput() {
    ...
    input.addEventListener('input', () => {
      ...
      if (!AppState.tags.find(t => t.name.toLowerCase() === q)) {
        const create = document.createElement('div');
        create.className = 'suggestion-item suggestion-create';
        // Color dot + hidden color picker for the new tag
        let pendingTagColor = '#3b82f6';
        const colorDot = document.createElement('span');
        colorDot.style.cssText = 'width:14px;height:14px;border-radius:50%;background:#3b82f6;flex-shrink:0;cursor:pointer;border:1.5px solid rgba(0,0,0,0.15);display:inline-block;margin-right:6px;vertical-align:middle;';
        colorDot.title = '色を変更';
        const colorInput2 = document.createElement('input');
        colorInput2.type = 'color'; colorInput2.value = '#3b82f6';
        colorInput2.style.cssText = 'position:absolute;opacity:0;width:0;height:0;pointer-events:none;';
        colorInput2.addEventListener('input', e => { pendingTagColor = e.target.value; colorDot.style.background = pendingTagColor; e.stopPropagation(); });
        colorDot.addEventListener('click', e => { e.stopPropagation(); colorInput2.click(); });
        create.appendChild(colorDot);
        create.appendChild(colorInput2);
        const createLabel = document.createTextNode(`「${input.value.trim()}」を新規タグとして追加`);
        create.appendChild(createLabel);
        create.addEventListener('click', async () => {
          const name = input.value.trim();
          const tag  = { id: generateId(), name, color: pendingTagColor || '#3b82f6', displayOrder: AppState.tags.length, createdAt: Date.now() };
          AppState.addTag(tag);
