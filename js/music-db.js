...
function getMusicSearchKey(text) {
  return normalizeString(text || '');
}

function scoreMusicCandidate(queryNorm, music) {
  const titleNorm = getMusicSearchKey(music.title);
  const pronunciationNorm = getMusicSearchKey(music.pronunciation || '');
  const titleDist = titleNorm ? levenshtein(queryNorm, titleNorm) / Math.max(queryNorm.length, titleNorm.length, 1) : 1;
  const pronDist = pronunciationNorm ? levenshtein(queryNorm, pronunciationNorm) / Math.max(queryNorm.length, pronunciationNorm.length, 1) : 1;

  let score = Math.min(titleDist, pronDist);
  if (titleNorm === queryNorm) score = -3;
  else if (pronunciationNorm && pronunciationNorm === queryNorm) score = -2.8;
  else if (titleNorm.startsWith(queryNorm)) score = Math.min(score, 0.05);
  else if (pronunciationNorm && pronunciationNorm.startsWith(queryNorm)) score = Math.min(score, 0.1);
  else if (titleNorm.includes(queryNorm)) score = Math.min(score, 0.2);
  else if (pronunciationNorm && pronunciationNorm.includes(queryNorm)) score = Math.min(score, 0.25);
...
// 手入力時の候補表示用。曲名(title)と読み方(pronunciation)の両方で検索し、
// ひらがな入力にも対応する。
function findMusicTitleCandidates(query, limit = 8) {
  if (!dbMusics || dbMusics.length === 0) return [];
  const queryNorm = getMusicSearchKey(query);
  if (!queryNorm) return [];
  return dbMusics
    .map(m => scoreMusicCandidate(queryNorm, m))
    .sort((a, b) => a.score - b.score || a.music.title.localeCompare(b.music.title, 'ja'))
    .slice(0, limit);
}

// 入力値から最も有力な曲候補を返す。候補が弱すぎる場合は null。
function findBestMusicInputMatch(query) {
  const cand = findMusicTitleCandidates(query, 1)[0] || null;
  if (!cand) return null;
  return (cand.score <= 0.35) ? cand : null;
}
