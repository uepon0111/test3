...
async function loadMusicDb() {
  try {
    ...
    showMusicDbLoadFailureNotice();
  }
}

// 楽曲マスターDBの取得に失敗した場合の通知。曲名オートコンプリート(かな検索含む)や
// OCR結果の曲名照合が利用できなくなるため、原因不明のまま機能低下したように見えない
// よう、画面上部に理由を明示して知らせる(notification-area の既存スタイルを流用)。
function showMusicDbLoadFailureNotice() {
  const area = document.getElementById('notification-area');
  if (!area) return;
  area.style.display = 'flex';
  const el = document.createElement('div');
  el.className = 'best-notification db-load-error';
  el.innerHTML = `
    <span class="material-symbols-outlined best-notification-icon">cloud_off</span>
    <div class="best-notification-body">
      <div class="best-notification-title">楽曲マスターDBの取得に失敗しました</div>
      <div class="best-notification-detail">曲名の候補表示(かな検索)やOCR結果の自動照合が利用できません。通信状況をご確認のうえ、ページを再読み込みしてください。</div>
    </div>
