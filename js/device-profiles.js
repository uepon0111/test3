...
// skipSync=true は、Driveから取得した内容をそのままローカルに反映する場合など、
// 「今まさにDriveの内容を書き込もうとしている/読み込んだばかり」で自動同期が
// 不要(または重複送信になってしまう)なケースのために使う内部フラグ。
// 通常の保存(プロファイルの保存・削除・インポート)では常に自動同期される。
function saveDeviceProfiles(list, skipSync) {
  try {
    localStorage.setItem(LS_KEY_DEVICE_PROFILES, JSON.stringify(list));
    deviceProfilesCache = list;
    if (!skipSync && typeof requestSettingsSync === 'function') requestSettingsSync();
    return true;
  } catch (e) {
