...
let deviceProfilesSyncTimer = null;
let deviceProfilesSyncInFlight = false;
let deviceProfilesSyncDisabled = false;

function saveDeviceProfiles(list, opts = {}) {
  try {
    localStorage.setItem(LS_KEY_DEVICE_PROFILES, JSON.stringify(list));
    deviceProfilesCache = list;
    if (!opts.skipDriveSync && !deviceProfilesSyncDisabled) {
      scheduleDeviceProfilesSync();
    }
    return true;
  } catch (e) {
...
function scheduleDeviceProfilesSync() {
  if (deviceProfilesSyncDisabled) return;
  if (deviceProfilesSyncTimer) clearTimeout(deviceProfilesSyncTimer);
  deviceProfilesSyncTimer = setTimeout(() => {
    syncDeviceProfilesToDrive().catch(err => console.error('機種プロファイルのDrive同期に失敗しました', err));
  }, 700);
}
