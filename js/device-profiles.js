...
function normalizeProfile(profile) {
  const base = profile || {};
  const regions = cloneRegions(base.regions);
  for (const def of REGION_DEFS) {
    if (!regions[def.key]) regions[def.key] = cloneRegions({ [def.key]: DEFAULT_REGIONS[def.key] })[def.key];
  }
  return {
    id: base.id || generateProfileId(),
    name: base.name || '新しい機種',
    width: toInt(base.width, 0),
    height: toInt(base.height, 0),
    regions,
...
function importProfilesJSON(jsonStr) {
  ...
  incoming.forEach(p => {
    ...
    const idx = merged.findIndex(e => e.id === normalized.id);
    if (idx >= 0) { merged[idx] = normalized; updated++; }
    else { merged.push(normalized); added++; }
  });
