...
let activeRegionKey = null;     // 現在編集対象の範囲キー ('difficulty' | 'title' | 'level' | 'breakdown' | 'combo')
let sampleNaturalWidth = 0;
let sampleNaturalHeight = 0;
let dragState = null;

function configureBackLink() {
  const link = document.querySelector('.btn-back-link');
  if (!link) return;
  if (window.parent && window.parent !== window) {
    link.innerHTML = '<span class="material-symbols-outlined">close</span> 閉じる';
    link.href = '#';
    link.addEventListener('click', (e) => {
      e.preventDefault();
      if (window.parent && typeof window.parent.closeSettingsOverlay === 'function') {
        window.parent.closeSettingsOverlay();
      }
    });
  }
}
