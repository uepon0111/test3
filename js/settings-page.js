...
function initSettingsPage() {
  ...
  const runOcrBtn = document.getElementById('btn-run-ocr-test');
  if (runOcrBtn) runOcrBtn.addEventListener('click', runSampleOcrTest);
...
async function handleSampleImageSelected(e) {
  ...
  const summaryEl = document.getElementById('ocr-debug-summary');
  const logEl = document.getElementById('ocr-debug-log');
  if (summaryEl) summaryEl.textContent = 'OCRテストを実行すると、この画像の実測ログが表示されます。';
