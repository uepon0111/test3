...
function openSettingsModal() {
  if (!isGoogleSignedIn()) {
    // 読み取り設定はGoogleアカウントに紐づけて同期するため、ログイン前は開けない
    // (ボタン自体もログイン前は非表示だが、旧settings.htmlからのリダイレクトなど
    // ログイン前に openSettingsModal が直接呼ばれる経路もあるため、ここでも防御する)。
    alert('読み取り設定を利用するには、先にGoogleでログインしてください。');
    return;
  }
...
function initSettingsPage() {
  ...
  // 設定変更ごとの自動Drive同期の状態を表示する (syncing/synced/error)。
  onSettingsSyncStatusChange(updateSettingsSyncIndicator);
  updateSettingsSyncIndicator(settingsSyncStatus);
}

// 設定のDrive自動同期状態インジケータ。device-profiles.js の保存操作(saveCurrentProfile /
// deleteCurrentProfile / doImportProfiles)のたびに drive-settings-sync.js から呼ばれる。
function updateSettingsSyncIndicator(status) {
  const el = document.getElementById('settings-sync-status');
  if (!el) return;
  const map = {
    idle: { text: '', cls: '' },
    syncing: { text: 'Google Driveに同期中…', cls: 'sync-syncing' },
    synced: { text: 'Google Driveに同期済み', cls: 'sync-synced' },
    error: { text: '同期エラー(この端末には保存されています)', cls: 'sync-error' },
  };
  const m = map[status] || map.idle;
  el.innerText = m.text;
  el.className = 'settings-sync-status ' + m.cls;
}
...
function onRegionPointerDown(ev, key) {
  ...
  showMagnifier(ev.clientX, ev.clientY);
}
...
// ドラッグ中の拡大鏡
// -----------------------------------------------------------------------
// ドラッグ/リサイズ中は指やカーソルで対象の境界そのものが隠れてしまい、細かい
// 位置合わせがしづらいため、周辺を拡大した表示をポインタ付近に重ねて出す。
// サンプル画像が選択されていない場合は拡大しようがないので何もしない。
// ============================================================

const MAGNIFIER_SIZE = 176; // 表示する正方形のCSSピクセルサイズ
const MAGNIFIER_ZOOM = 3;   // 拡大倍率

function ensureMagnifier() {
  let mag = document.getElementById('region-magnifier');
  if (mag) return mag;
  mag = document.createElement('div');
  mag.id = 'region-magnifier';
  mag.className = 'region-magnifier';
  mag.innerHTML = `<canvas id="region-magnifier-canvas" width="${MAGNIFIER_SIZE}" height="${MAGNIFIER_SIZE}"></canvas><div class="region-magnifier-crosshair"></div>`;
...
function showMagnifier(clientX, clientY) {
  const img = document.getElementById('sample-img');
  if (!sampleNaturalWidth || !sampleNaturalHeight || !img || img.style.display === 'none') return;

  const mag = ensureMagnifier();
  const imgRect = img.getBoundingClientRect();

  // 指/カーソルの真下に重ならないよう右上方向にオフセットして配置する
  // (タッチ操作で指自体が対象を隠してしまうのを防ぐ、スマートフォンのテキスト選択
  // ルーペと同様の配慮)。画面端では収まる向きへ自動的に切り替える。
  let left = clientX + 28;
  let top = clientY - MAGNIFIER_SIZE - 28;
  if (top < 8) top = clientY + 28;
  if (left + MAGNIFIER_SIZE > window.innerWidth - 8) left = clientX - MAGNIFIER_SIZE - 28;
  if (left < 8) left = 8;
  mag.style.left = left + 'px';
  mag.style.top = top + 'px';
  mag.style.display = 'block';

  const canvas = document.getElementById('region-magnifier-canvas');
  const ctx = canvas.getContext('2d');
  ctx.imageSmoothingEnabled = false; // 拡大時に文字の縁がにじまないようにする(境界を確認しやすくする)

  const relX = clamp((clientX - imgRect.left) / imgRect.width, 0, 1);
  const relY = clamp((clientY - imgRect.top) / imgRect.height, 0, 1);
  const nx = relX * sampleNaturalWidth;
  const ny = relY * sampleNaturalHeight;
  const srcSize = MAGNIFIER_SIZE / MAGNIFIER_ZOOM;
  const sx = clamp(nx - srcSize / 2, 0, Math.max(0, sampleNaturalWidth - srcSize));
  const sy = clamp(ny - srcSize / 2, 0, Math.max(0, sampleNaturalHeight - srcSize));

  ctx.clearRect(0, 0, MAGNIFIER_SIZE, MAGNIFIER_SIZE);
  ctx.drawImage(img, sx, sy, srcSize, srcSize, 0, 0, MAGNIFIER_SIZE, MAGNIFIER_SIZE);

  // ドラッグ中の範囲の枠を拡大表示にも重ねて描き、境界が画素のどこに
  // あたっているか見ながら微調整できるようにする。
  if (dragState && currentProfile) {
    const region = currentProfile.regions[dragState.key];
    const def = REGION_DEFS.find(d => d.key === dragState.key);
    const rx = region.x * sampleNaturalWidth, ry = region.y * sampleNaturalHeight;
    const rw = region.w * sampleNaturalWidth, rh = region.h * sampleNaturalHeight;
    ctx.strokeStyle = (def && def.color) || '#007bff';
    ctx.lineWidth = 2;
    ctx.strokeRect((rx - sx) * MAGNIFIER_ZOOM, (ry - sy) * MAGNIFIER_ZOOM, rw * MAGNIFIER_ZOOM, rh * MAGNIFIER_ZOOM);
  }
}

function hideMagnifier() {
  const mag = document.getElementById('region-magnifier');
