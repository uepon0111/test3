...
const persistCurrentProfile = debounce(() => {
  if (currentProfile) upsertDeviceProfile(currentProfile);
}, 300);

function getDragMagnifier() {
  return document.getElementById('drag-magnifier');
}

function updateDragMagnifier(ev) {
  const magnifier = getDragMagnifier();
  const img = document.getElementById('sample-img');
  const container = document.getElementById('image-editor-container');
  if (!magnifier || !img || !container || img.style.display === 'none' || !currentProfile) return;
  const imgRect = img.getBoundingClientRect();
  const containerRect = container.getBoundingClientRect();
  const x = clamp(ev.clientX - imgRect.left, 0, imgRect.width);
  const y = clamp(ev.clientY - imgRect.top, 0, imgRect.height);
  const zoom = 2.6;
  const size = 180;
  magnifier.style.display = 'block';
  magnifier.style.left = `${clamp((ev.clientX - containerRect.left) + 24, 0, Math.max(0, container.clientWidth - size))}px`;
  magnifier.style.top = `${clamp((ev.clientY - containerRect.top) - size - 24, 0, Math.max(0, container.clientHeight - size))}px`;
...
function initSettingsPage() {
  ...
  document.getElementById('profile-name-input').addEventListener('input', () => {
    ...
    currentProfile.name = document.getElementById('profile-name-input').value;
    renderProfileList();
