...
function openSettingsModal() {
  ...
  if (!settingsPageInitialized) {
    ...
    // 2回目以降の表示時も、閉じている間の同期(Google Driveからの取り込みなど)や
    // 他端末での変更を反映するため、一覧と選択中の内容を読み直す。
    refreshProfileEditorFromStorage();
    repositionAllRegionBoxes();
  }
  onSettingsSyncStateChanged(settingsSyncState); // 直近の同期状態をバッジに反映
}

// Google Driveからの設定同期など、外部要因でプロファイル一覧が変わった際に呼ばれる
// (settings-sync.js から)。設定モーダルを一度も開いていなければ何もしない
// (次に開かれた際に openSettingsModal 側でどのみち最新値が使われるため)。
function onProfilesUpdatedExternally() {
  if (!settingsPageInitialized) return;
  refreshProfileEditorFromStorage();
}

// 現在選択中のプロファイルがまだ存在すればそれを維持しつつ最新の内容で読み直し、
// 無くなっていれば(他端末で削除された等)先頭のプロファイルを選び直す。
function refreshProfileEditorFromStorage() {
  const profiles = getDeviceProfiles();
  const stillExists = currentProfile && profiles.some(p => p.id === currentProfile.id);
  loadProfileIntoEditor(stillExists ? getDeviceProfileById(currentProfile.id) : profiles[0]);
}
