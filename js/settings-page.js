...
function setupEmbeddedCloseBehavior() {
  ...
  backLink.addEventListener('click', (e) => {
    e.preventDefault();
    try {
      window.parent.postMessage({ type: 'close-settings-modal' }, window.location.origin);
    } catch (err) {
      console.error(err);
    }
  });
