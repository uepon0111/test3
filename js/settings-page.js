function notifyParentSettingsUpdated() {
  try {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'prsk-device-profiles-updated' }, '*');
    }
  } catch (_) {}
}
...
function handleBackNavigation() {
  try {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'prsk-close-settings' }, '*');
      return;
    }
  } catch (_) {}
...
function initSettingsPage() {
  ...
  const backBtn = document.getElementById('btn-back-link');
