...
// selection-mode.js から呼ばれる軽量な再描画(絞り込み・並び替えはやり直さず、
// 現在の filteredRecords をそのまま再描画するだけ。選択モードの切り替え/選択解除で
// カードの見た目(選択枠・オーバーレイの表示)だけを更新したい場合に使う)。
function renderGrid(records) {
  renderGridChunked(records, ++renderToken);
}
...
function initFilterControls() {
  ...
  initTitleFilterAutocomplete();
}

// ============================================================
// 曲名検索欄のオートコンプリート (かな入力対応)
// ============================================================
// 候補は「現在読み込み済みのレコードに実際に存在する曲名」のみに限定する
// (1件も無い曲を候補に出しても絞り込み結果が0件になるだけで意味が無いため)。
// 読み方(ひらがな)はマスターDBから、musicId が分かっていればそれで、
// 分からない場合(旧データ等)は曲名のファジーマッチで補う。
function buildFilterTitleSource() {
  const seen = new Map();
  for (const rec of (allRecords || [])) {
    if (!rec.title || seen.has(rec.title)) continue;
    let pronunciation = '';
    if (rec.musicId != null) {
      const m = getMusicById(rec.musicId);
      if (m) pronunciation = m.pronunciation || '';
    }
    if (!pronunciation) {
      const bm = findBestMatchMusic(rec.title);
      if (bm && (bm.distanceRatio <= TITLE_MATCH_CONFIDENT_MAX_NORM_DIST || bm.distanceAbs <= TITLE_MATCH_CONFIDENT_MAX_ABS_DIST)) {
        pronunciation = bm.music.pronunciation || '';
      }
    }
    seen.set(rec.title, { title: rec.title, pronunciation });
  }
  return Array.from(seen.values());
}

function initTitleFilterAutocomplete() {
  const inputEl = document.getElementById('filter-title');
  initTitleAutocomplete({
    inputEl,
    getSource: buildFilterTitleSource,
    onSelect: (it) => {
