...
// アクセストークンを取得する。未ログイン時はエラーにする。
function getDriveAccessTokenOrThrow() {
  const token = gapi.client.getToken();
  if (!token || !token.access_token) throw new Error('Google にログインしていません');
  return token.access_token;
}

async function fetchDriveTextFile(fileId) {
  const token = getDriveAccessTokenOrThrow();
  const res = await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?alt=media`, {
    headers: new Headers({ 'Authorization': 'Bearer ' + token }),
  });
  if (!res.ok) throw new Error(`Drive のファイル取得に失敗しました (${res.status})`);
...
async function createDriveObjectUrl(fileId) {
  const blob = await fetchDriveMediaBlob(fileId);
  return URL.createObjectURL(blob);
}

async function findDriveFileByNameInFolder(name, folderId, mimeType = null) {
  let query = `name = '${escapeDriveQueryValue(name)}' and trashed = false`;
  if (folderId) query += ` and '${folderId}' in parents`;
  if (mimeType) query += ` and mimeType = '${mimeType}'`;
  const response = await gapi.client.drive.files.list({ q: query, fields: 'files(id, name, mimeType, modifiedTime)', pageSize: 1 });
  return (response.result.files && response.result.files.length > 0) ? response.result.files[0] : null;
}

async function uploadMultipartBlobToDrive({ parentId, fileName, blob, mimeType, fileId = null, fields = 'id,name,modifiedTime' }) {
  const token = getDriveAccessTokenOrThrow();
  ...
  const url = fileId
    ? `https://www.googleapis.com/upload/drive/v3/files/${fileId}?uploadType=multipart&fields=${fields}`
  ...
  return await new Promise((resolve, reject) => {
    ...
    xhr.onerror = () => reject(new Error('Drive へのアップロードに失敗しました'));
    xhr.send(form);
  });
}


async function uploadFileToResults(file, properties, fileName, onProgress) {
  ...
  const res = await new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,parents', true);
    xhr.setRequestHeader('Authorization', 'Bearer ' + token);
...
// 設定同期ファイルを検索する。Drive 内のこのアプリのルートフォルダ配下のみを見る。
async function findProfilesSyncFile(rootId) {
  return await findDriveFileByNameInFolder(DEVICE_PROFILES_SYNC_FILE_NAME, rootId, 'application/json');
}
