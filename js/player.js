...
const PlayerScreen = (() => {
  ...
  const PlayerWidgetUI = {
    ...
    updateSpeed() {
      ...
      // Update both dropdown displays and option highlights
      document.querySelectorAll('.speed-current').forEach(el => { el.textContent = `${v}×`; });
  ...
  function _setCtrlIcon(btnId, iconName) {
    const btn = el(btnId);
    if (!btn) return;
    // Lucideは初回描画後 <i> を <svg> に置き換えるため innerHTML で再生成する
  ...
  /* ── Progress bar seek (pointer events + immediate visual feedback) ── */
  function _bindProgressBar(barId) {
    const bar = el(barId);
    if (!bar) return;
    const fillId = barId + '-fill'; // pw-progress-fill / po-progress-fill
    let dragging = false;
  ...
  /* ── Speed panel toggle ── */
  function _bindSpeedPanel(toggleId, panelId) {
    const tog   = el(toggleId);
  ...
  function _bindPlayerControls() {
    ...
    _bindSpeedPanel('pw-speed-toggle', 'pw-speed-panel');
    ...
    _bindSpeedPanel('po-speed-toggle', 'po-speed-panel');
