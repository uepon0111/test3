...
const PlayerScreen = (() => {
  ...
  const PlayerWidgetUI = {
    ...
    updateProgress() {
      ...
      _setProgress('pw-progress-fill', 'pw-progress-thumb', 'pw-progress', 'pw-cur-time', 'pw-tot-time', pct, cur, dur);
      _setProgress('po-progress-fill', 'po-progress-thumb', 'po-progress', 'po-cur-time', 'po-tot-time', pct, cur, dur);
    },
    ...
    updateSpeed() {
      const v = String(AppState.playbackRate);
      const pw = el('pw-speed-select'); if (pw) pw.value = v;
      const po = el('po-speed-select'); if (po) po.value = v;
    },
  ...
  function _setCtrlIcon(btnId, iconName) {
    ...
    if (!ico) return;
    ico.dataset.lucide = iconName;
    lucide.createIcons({ elements: [btn] });
  }
  function _setProgress(fillId, thumbId, barId, curId, totId, pct, cur, dur) {
    ...
    if (bar) {
      ...
      bar.setAttribute('aria-valuetext', `${formatTime(cur)} / ${formatTime(dur)}`);
    }
  ...
  function _bindProgressBar(barId) {
    ...
    bar.addEventListener('pointerdown', e => {
      ...
      AudioEngine.seek(calc(e));
    });
    bar.addEventListener('pointermove', e => { if (dragging) AudioEngine.seek(calc(e)); });
    ...
    bar.addEventListener('keydown', e => {
      ...
      if (['ArrowLeft', 'ArrowDown'].includes(e.key)) {
        e.preventDefault();
        AudioEngine.seek(((AppState.currentTime - step) / Math.max(AppState.duration, 1)) * 100);
        ...
        e.preventDefault();
        AudioEngine.seek(100);
      }
    });
  }

  /* ── Speed buttons ── */
  function _bindSpeedSelect(selectId) {
    const sel = el(selectId);
    if (!sel) return;
    sel.addEventListener('change', () => {
      AudioEngine.setSpeed(parseFloat(sel.value));
      PlayerWidgetUI.updateSpeed();
    });
    sel.value = String(AppState.playbackRate);
  }
  ...
  function _bindPlayerControls() {
    ...
    _bindSpeedSelect('pw-speed-select');
    ...
    _bindSpeedSelect('po-speed-select');
