...
// スコープはこのアプリが作成したファイル/フォルダのみにアクセスできる drive.file に縮小しています。
// (以前は drive を利用していましたが、Drive全体を見せる必要は無いため)
// このアプリが過去に作成したフォルダ・ファイルは、作成時のスコープに関わらずこのスコープでも
// 引き続きアクセスできます(Driveは「どのアプリが作成したか」をファイル単位で記録しているため)。
// 一方、スコープをこのアプリの過去バージョンより広く要求していた期間の「ユーザーが別途Drive上で
// 直接操作した」ファイルまでは対象になりません。既存ユーザーは、スコープ変更に伴い初回のみ
// 再ログイン(再同意)が必要になる場合があります。
...
// 機種プロファイル(読み取り設定)をアカウントに紐づけて同期するための設定ファイル名。
// ルートフォルダ直下に置く(Resultsフォルダとは別)。
