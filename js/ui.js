...
const UI = (() => {
  ...
  let _highlightPeriod = '1m'; // period for the highlight recap section
  ...
  async function renderLogOverview() {
    ...
    _renderHighlight(tracks, logs, _highlightPeriod);
  }
  ...
  /* ─────────────────────────────────────────
     LOG HIGHLIGHT (period recap)
  ───────────────────────────────────────── */
  function _getHighlightRange(period) {
    const now = Date.now();
    switch (period) {
  ...
  function _highlightPeriodLabel(period) {
    const now   = new Date();
    const fmt   = d => `${d.getFullYear()}/${d.getMonth()+1}/${d.getDate()}`;
    const since = new Date(_getHighlightRange(period));
    switch (period) {
      case '1m':   return `過去1ヶ月 (${fmt(since)} 〜 ${fmt(now)})`;
      case '3m':   return `過去3ヶ月 (${fmt(since)} 〜 ${fmt(now)})`;
      case '6m':   return `過去6ヶ月 (${fmt(since)} 〜 ${fmt(now)})`;
      case 'year': return `${now.getFullYear()}年`;
      default:     return '全期間';
    }
  }

  function _renderHighlight(tracks, logs, period) {
    const body = document.getElementById('log-highlight-body');
    if (!body) return;

    const since    = _getHighlightRange(period);
    const filtered = logs.filter(l => l.playedAt >= since);

    if (filtered.length === 0) {
      body.innerHTML = `<p style="color:var(--text-muted);font-size:13px;padding:12px 0">この期間の再生記録がありません</p>`;
      return;
    }
    ...
    // ─── aggregate ──────────────────────────────────────
    const trackTime  = new Map(); // trackId → seconds
    const trackPlays = new Map(); // trackId → count
    const artistTime = new Map(); // artist  → seconds

    filtered.forEach(l => {
      const dur = l.duration || 0;
      trackTime.set(l.trackId,  (trackTime.get(l.trackId)  || 0) + dur);
      trackPlays.set(l.trackId, (trackPlays.get(l.trackId) || 0) + 1);
      const t = trackMap.get(l.trackId);
      if (t?.artist) {
        artistTime.set(t.artist, (artistTime.get(t.artist) || 0) + dur);
      }
    });
    ...
    // Top tracks (by play time, fall back to play count)
    const topTracks = [...trackTime.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([id, dur]) => ({ track: trackMap.get(id), dur, plays: trackPlays.get(id) || 0 }))
      .filter(x => x.track);
    ...
    const topArtists = [...artistTime.entries()]
      ...
      .slice(0, 5)
      .map(([artist, dur]) => {
        // Find any track by this artist that has a thumbnail
        const rep = tracks.find(t => t.artist === artist && t._thumbUrl) ||
                    tracks.find(t => t.artist === artist);
        return { artist, dur, thumbUrl: rep?._thumbUrl || null };
      });

    // ─── build HTML ──────────────────────────────────────
    const thumbImg = (url, fallbackIcon = 'fa-music') => url
      ? `<img src="${url}" style="width:100%;height:100%;object-fit:cover;border-radius:inherit">`
      : `<i class="fa-solid ${fallbackIcon}" style="font-size:18px;color:var(--accent);opacity:0.6"></i>`;

    // Artists section
    const [featArtist, ...restArtists] = topArtists;
    const featArtistHtml = featArtist ? `
      <div class="hl-feature-card">
        <div class="hl-feature-thumb">
          ${thumbImg(featArtist.thumbUrl, 'fa-user')}
        </div>
        <div class="hl-feature-label">${_esc(featArtist.artist)}</div>
        <div class="hl-feature-sub">${_fmtSec(featArtist.dur)}</div>
      </div>` : '';

    const restArtistsHtml = restArtists.map((a, i) => `
      <div class="hl-list-item">
        <span class="hl-list-rank">${i + 2}</span>
        <div class="hl-list-thumb">
          ${thumbImg(a.thumbUrl, 'fa-user')}
        </div>
        <div class="hl-list-info">
          <div class="hl-list-title">${_esc(a.artist)}</div>
          <div class="hl-list-sub">${_fmtSec(a.dur)}</div>
        </div>
      </div>`).join('');

    // Tracks section
    const [featTrack, ...restTracks] = topTracks;
    const featTrackHtml = featTrack ? `
      <div class="hl-feature-card">
        <div class="hl-feature-thumb">
          ${thumbImg(featTrack.track._thumbUrl)}
        </div>
        <div class="hl-feature-label">${_esc(featTrack.track.title)}</div>
        <div class="hl-feature-sub">${_esc(featTrack.track.artist)} · ${featTrack.plays}回</div>
      </div>` : '';

    const restTracksHtml = restTracks.map((x, i) => `
      <div class="hl-list-item">
        <span class="hl-list-rank">${i + 2}</span>
        <div class="hl-list-thumb">
          ${thumbImg(x.track._thumbUrl)}
        </div>
        <div class="hl-list-info">
          <div class="hl-list-title">${_esc(x.track.title)}</div>
          <div class="hl-list-sub">${_esc(x.track.artist)} · ${x.plays}回</div>
        </div>
      </div>`).join('');

    const totalHtml = totalSec > 0
      ? (totalH > 0 ? `${totalH}<span class="hl-total-unit">時間</span>${totalMin}<span class="hl-total-unit">分</span>`
                    : `${totalMin}<span class="hl-total-unit">分</span>`)
      : '0<span class="hl-total-unit">分</span>';

    body.innerHTML = `
      ${topArtists.length > 0 ? `
      <div class="hl-section">
        <div class="hl-section-title"><i class="fa-solid fa-user-music"></i>よく聴いたアーティスト</div>
        <div class="hl-grid">
          ${featArtistHtml}
          <div class="hl-list">${restArtistsHtml}</div>
        </div>
      </div>` : ''}
      ${topTracks.length > 0 ? `
      <div class="hl-section">
        <div class="hl-section-title"><i class="fa-solid fa-music"></i>よく聴いた曲</div>
        <div class="hl-grid">
          ${featTrackHtml}
          <div class="hl-list">${restTracksHtml}</div>
        </div>
      </div>` : ''}
      <div class="hl-total-bar">
        <div class="hl-total-label"><i class="fa-solid fa-clock"></i>総再生時間</div>
        <div class="hl-total-value">${totalHtml}</div>
      </div>`;
  }

  function switchHighlightPeriod(btn) {
    document.getElementById('highlight-period-tabs')
      ?.querySelectorAll('.inner-tab')
      .forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    _highlightPeriod = btn.dataset.period;
    // Re-render highlight only — avoid full overview re-fetch
    // FIX: add .catch() to prevent unhandled Promise rejection if getLogs() fails.
    Storage.getLogs()
      .then(logs => _renderHighlight(_tracks, logs, _highlightPeriod))
      .catch(err => console.warn('[UI] switchHighlightPeriod getLogs failed:', err));
  }
  ...
  function _renderBarChart(tracks, logs) {
    ...
    // PERF FIX: Pre-cache Date objects for each log entry to avoid repeated
    // construction inside filter/forEach closures below.
    const logDates = logs.map(l => new Date(l.playedAt));
    ...
    if (_graphPeriod === 'month') {
      ...
      logs.forEach((l, i) => {
        if (logDates[i].getFullYear() === year)
          data[logDates[i].getMonth()] += l.duration || 0;
      });
      periodLabel = year + '年';
      ...
      logs.forEach((l, i) => {
        if (l.playedAt >= wsT && l.playedAt <= weT)
          data[logDates[i].getDay()] += l.duration || 0;
      });
      ...
      logs.forEach((l, i) => {
        const y = String(logDates[i].getFullYear());
        const idx = labels.indexOf(y);
        if (idx >= 0) data[idx] += l.duration || 0;
      });
      periodLabel = `${labels[0]}年 〜 ${labels[4]}年`;
      isAtPresent = centerY + 2 >= now.getFullYear();
    }

    // For 'era' category, overlay release-date distribution as a secondary visual cue
    // by highlighting bars where most played tracks' era matches
  ...
  function _renderRanking(tracks, logs) {
    ...
    if (_graphCat === 'artist') {
      const artistDur = new Map();
      logs.forEach(l => {
        const t = trackMap.get(l.trackId);
        if (t?.artist) artistDur.set(t.artist, (artistDur.get(t.artist)||0) + (l.duration||0));
      });
      const sorted = [...artistDur.entries()].sort((a,b)=>b[1]-a[1]).slice(0,10);
      if (sorted.length === 0) {
        rank.innerHTML = '<p style="color:var(--text-muted);font-size:13px;padding:8px 0">再生履歴がありません</p>';
        return;
      }
      rank.innerHTML = sorted.map(([artist, dur], i) => `
        <div class="ranking-item">
          <span class="ranking-num ${i<3?'top':''}">${i+1}</span>
          <span class="ranking-name">${_esc(artist || '不明なアーティスト')}</span>
    ...
    if (_graphCat === 'tag') {
      const tagDur = new Map();
      logs.forEach(l => {
        const t = trackMap.get(l.trackId);
        (t?.tags || []).forEach(tagId => {
          tagDur.set(tagId, (tagDur.get(tagId)||0) + (l.duration||0));
        });
      });
      const sorted = [...tagDur.entries()].sort((a,b)=>b[1]-a[1]).slice(0,10);
      if (sorted.length === 0) {
        rank.innerHTML = '<p style="color:var(--text-muted);font-size:13px;padding:8px 0">再生履歴がありません</p>';
        return;
      }
      rank.innerHTML = sorted.map(([tagId, dur], i) => {
        const tag = _tags.find(g => g.id === tagId);
        return `<div class="ranking-item">
          <span class="ranking-num ${i<3?'top':''}">${i+1}</span>
          ${tag ? `<span class="tag-dot" style="background:${tag.color};width:10px;height:10px;border-radius:50%;flex-shrink:0"></span>` : ''}
          <span class="ranking-name">${_esc(tag?.name || '削除済みタグ')}</span>
          <span class="ranking-val">${_fmtSec(dur)}</span>
        </div>`;
      }).join('');
      return;
    ...
    if (_graphCat === 'era') {
      const eraDur = new Map();
      logs.forEach(l => {
        const t = trackMap.get(l.trackId);
        const year = t?.releaseDate ? t.releaseDate.slice(0, 4) : '不明';
        eraDur.set(year, (eraDur.get(year)||0) + (l.duration||0));
      });
      const sorted = [...eraDur.entries()].sort((a,b)=>b[1]-a[1]).slice(0,10);
      if (sorted.length === 0) {
        rank.innerHTML = '<p style="color:var(--text-muted);font-size:13px;padding:8px 0">再生履歴がありません</p>';
        return;
      }
      rank.innerHTML = sorted.map(([year, dur], i) => `
        <div class="ranking-item">
          <span class="ranking-num ${i<3?'top':''}">${i+1}</span>
          <span class="ranking-name">${_esc(year)}年代</span>
          <span class="ranking-val">${_fmtSec(dur)}</span>
        </div>`).join('');
      return;
    }

    // ── default: total — top tracks by play time ─────────────────────
  ...
  function _calDayClick(day) {
    ...
    // BUG FIX: guard against missing child elements to prevent TypeError.
    if (etitle) etitle.innerHTML = `<i class="fa-solid fa-calendar-day"></i>${_calYear}年${_calMonth+1}月${day}日 のイベント`;
    if (elist) elist.innerHTML = events.map(({ track, type }) => {
