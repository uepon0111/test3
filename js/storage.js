...
const Storage = (() => {
  ...
  /**
   * Import a snapshot from Drive.
   * FIX: Accepts an optional deletedIdSet (Set) so that tracks which were
   * deleted locally are not re-created from an outdated remote index.
   * Previously, if _pushIndex() had failed after a local deletion, the remote
   * index still contained the deleted track and importSnapshot would silently
   * re-create it, causing it to reappear in the UI.
   */
  async function importSnapshot(snapshot, deletedIdSet = new Set()) {
    if (!snapshot || snapshot.version !== 1) return;
    for (const t of snapshot.tracks||[]) {
      if (deletedIdSet.has(t.id)) continue;   // never re-import deleted tracks
  ...
  function _imgMime(b, o=0) {
    ...
    if (b[o]===0x52 && b[o+1]===0x49 && b[o+2]===0x46 && b[o+4]===0x57) return 'image/webp';
    return 'image/jpeg';
  }
