...
function reconcileOcrResult(raw) {
  ...
  // 注意: プロセカでは GOOD 判定でもコンボが途切れる(フルコンボの条件は GOOD/BAD/MISS が
  // 全て0件で、GREAT以上のみで叩き切ること)。BAD・MISSだけを見て判定すると、GOODが
  // 1件以上あるのに「フルコンボのはず」と誤認して誤警告を出してしまうため、
  // 必ず good === 0 も条件に含める。
  ...
  if (totalNotes > 0 && bd.good === 0 && bd.bad === 0 && bd.miss === 0 && raw.combo.value !== totalNotes) {
    warnings.push({
      level: 'warn', field: 'combo',
      message: `GOOD・BAD・MISSが0件(フルコンボ)にもかかわらず、コンボ数(${raw.combo.value})が総ノーツ数(${totalNotes})と一致していません。読み取り結果をご確認ください。`,
    });
  }
...
function checkManualEditWarnings(data) {
  ...
  // プロセカでは GOOD 判定でもコンボが途切れる(フルコンボの条件は GOOD/BAD/MISS が全て0件)。
  // bad・missだけでなく good も0件の場合に限り「フルコンボのはず」とみなす。
  if (totalNotes > 0 && good === 0 && bad === 0 && miss === 0 && comboValue !== totalNotes) {
    warnings.push({
      level: 'warn', field: 'combo',
      message: `GOOD・BAD・MISSが0件(フルコンボ)にもかかわらず、コンボ数(${comboValue})が総ノーツ数(${totalNotes})と一致していません。入力値をご確認ください。`,
    });
  }
